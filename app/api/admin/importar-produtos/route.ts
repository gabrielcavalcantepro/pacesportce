  ...
  try {
    ...
    for (let i = 0; i < dataRows.length && produtos.length < MAX_PRODUTOS; i++) {
      ...
      const produto: ProdutoImportado = {
        ...
        estoque: parseNumber(row[7]) ?? 0,
        destaque: parseBoolean(row[8]),
        frete_gratis: parseBoolean(row[9]),
        whatsapp_only: parseBoolean(row[10]),
        peso: parseNumber(row[11]),
        comprimento: parseNumber(row[12]),
        largura: parseNumber(row[13]),
        altura: parseNumber(row[14]),
        descricao: row[15] ? String(row[15]).trim() : null,
        descricao_full: row[16] ? String(row[16]).trim() : null,
        especificacoes: parseEspecificacoes(row[17]),
        variantes: parseVariantes(row[18]),
